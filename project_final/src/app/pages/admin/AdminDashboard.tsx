...
import {
    ...
    Video, ExternalLink, Image as ImageIcon, CalendarDays,
} from "lucide-react";
import {
...
function generatePDFReport(deptReport: string, allAppts: Appointment[]) {
    ...
    const addStatsPage = (dept: string, list: Appointment[], isFirst: boolean) => {
    ...
    if (deptReport === "Reporte Global") {
        ...
        addStatsPage(deptReport, allAppts.filter(a => a.department === deptReport), true);
    }
    ...
    autoTable(doc, {
        ...
        body: finalList.slice(0, 100).map(a => [a.studentName, a.specialistName, a.date, a.time, a.status]),
        styles: { fontSize: 8 },
        headStyles: { fillColor: [30, 41, 59] },
        margin: { left: 20, right: 20 },
    });
...
export function AdminDashboard() {
    ...
    const {
        ...
        events, addEvent, addResource, users, deleteUser,
    } = useStore();
    ...
    const deptChartData = useMemo(() => {
        ...
        depts.forEach(d => {
            ...
            dAppts.forEach(a => {
                const mon = new Date(a.date).toLocaleString("es-MX", { month: "short" });
    ...
    return (
        <AppShell sidebar={{ tabs: sidebarTabs, active: activeTab, onSelect: setActiveTab, badges: { citas: summary.pendientes } }}>
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden min-h-[500px]">
                                    ...
                                    <tbody className="divide-y divide-slate-100 bg-white">
                                        {filteredAppts.map(cita => (
                                            <tr key={cita.id} className="hover:bg-slate-50/80 transition-colors">
                                                ...
                                                <td className="px-6 py-4"><p className="text-slate-900 font-bold text-sm">{new Date(cita.date + "T12:00:00").toLocaleDateString("es-MX", { day: "numeric", month: "short" })}</p></td>
                    ...
                    {activeTab === "contenido" && (
                        <div className="p-8">
                            ...
                            <div className="space-y-5 bg-slate-50 border border-slate-200 rounded-3xl p-6 shadow-sm max-w-2xl">
